...
class DataValidation:
    ...
    def detect_dataset_drift(self, reference_df: DataFrame, current_df: DataFrame) -> bool:
        ...
        Description :   This method validates if drift is detected using Evidently 0.7.11
        ...
        try:
            ...
            logging.info(f"Data Drift Analysis Complete:")
            logging.info(f"Total features analyzed: {n_features}")
            logging.info(f"Features with drift detected: {n_drifted_features}")
            logging.info(f"Dataset drift status: {drift_status}")
            logging.info(f"Drift percentage: {(n_drifted_features/n_features)*100:.2f}%")
            
            # Optional: Log individual feature drift details
            if 'drift_by_columns' in drift_metrics:
                ...
                if drifted_features:
                    logging.info(f"Features with drift: {', '.join(drifted_features)}")
            
            return drift_status
            
        except KeyError as e:
            error_msg = f"Error accessing drift metrics - possibly incorrect Evidently version or structure: {e}"
            logging.error(error_msg)
            raise USvisaException(error_msg, sys) from e
            
        except Exception as e:
            error_msg = f"Unexpected error in drift detection: {str(e)}"
            logging.error(error_msg)
            raise USvisaException(error_msg, sys) from e
